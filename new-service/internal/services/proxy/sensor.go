...
func (s *Device) processSensorInfo(ctx context.Context, dataRaw json.RawMessage) error {
	...
	dynamicInfo, err := s.dynamicSensorInfo(ctx, dataRaw)
	if err != nil {
...
func (s *Device) dynamicSensorInfo(ctx context.Context, dataRaw json.RawMessage) (*core.SensorInfoDynamic, error) {
	logger := logging.WithCtxFields(ctx)

	sensorInfoDynamic, err := s.sensorMapper.ConvertToSensorInfoDynamic(ctx, dataRaw)
	if err != nil {
