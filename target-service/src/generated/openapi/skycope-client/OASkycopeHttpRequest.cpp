...
OASkycopeHttpRequestInput::OASkycopeHttpRequestInput() {
    initialize();
}

OASkycopeHttpRequestInput::OASkycopeHttpRequestInput(QString v_url_str, QString v_http_method) {
    initialize();
    url_str = v_url_str;
    http_method = v_http_method;
}

void OASkycopeHttpRequestInput::initialize() {
    var_layout = NOT_SET;
    url_str = "";
    http_method = "GET";
}

void OASkycopeHttpRequestInput::add_var(QString key, QString value) {
    vars[key] = value;
}

void OASkycopeHttpRequestInput::add_file(QString variable_name, QString local_filename, QString request_filename, QString mime_type) {
    OASkycopeHttpFileElement file;
...
OASkycopeHttpRequestWorker::OASkycopeHttpRequestWorker(QObject *parent, QNetworkAccessManager *_manager)
...
OASkycopeHttpRequestWorker::~OASkycopeHttpRequestWorker() {
...
QMap<QString, QString> OASkycopeHttpRequestWorker::getResponseHeaders() const {
    return headers;
}

OASkycopeHttpFileElement OASkycopeHttpRequestWorker::getHttpFileElement(const QString &fieldname) {
    if (!files.isEmpty()) {
    ...
    return OASkycopeHttpFileElement();
}

QByteArray *OASkycopeHttpRequestWorker::getMultiPartField(const QString &fieldname) {
...
void OASkycopeHttpRequestWorker::setTimeOut(int timeOutMs) {
...
void OASkycopeHttpRequestWorker::setWorkingDirectory(const QString &path) {
    if (!path.isEmpty()) {
        workingDirectory = path;
    }
}

void OASkycopeHttpRequestWorker::setResponseCompressionEnabled(bool enable) {
    isResponseCompressionEnabled = enable;
}

void OASkycopeHttpRequestWorker::setRequestCompressionEnabled(bool enable) {
    isRequestCompressionEnabled = enable;
}

int  OASkycopeHttpRequestWorker::getHttpResponseCode() const{
    return httpResponseCode;
}

QString OASkycopeHttpRequestWorker::http_attribute_encode(QString attribute_name, QString input) {
...
void OASkycopeHttpRequestWorker::execute(OASkycopeHttpRequestInput *input) {
    ...
    if (input->var_layout == ADDRESS || input->var_layout == URL_ENCODED) {
        ...
        // add files
        for (QList<OASkycopeHttpFileElement>::iterator file_info = input->files.begin(); file_info != input->files.end(); file_info++) {
    ...
    if (input->request_body.size() > 0) {
        ...
        if(!isFormData && (input->var_layout != MULTIPART) && isRequestCompressionEnabled){
            request_content.append(compress(input->request_body, 7, OASkycopeCompressionType::Gzip));
        } else {
    ...
    if (OASkycopeHttpRequestWorker::sslDefaultConfiguration != nullptr) {
        request.setSslConfiguration(*OASkycopeHttpRequestWorker::sslDefaultConfiguration);
    }
    ...
    if (reply != nullptr) {
        ...
        connect(reply, &QNetworkReply::downloadProgress, this, &OASkycopeHttpRequestWorker::downloadProgress);
...
void OASkycopeHttpRequestWorker::on_reply_finished(QNetworkReply *reply) {
    bool codeSts = false;
...
void OASkycopeHttpRequestWorker::on_reply_timeout(QNetworkReply *reply) {
...
void OASkycopeHttpRequestWorker::process_response(QNetworkReply *reply) {
    ...
    if (!contentDispositionHdr.isEmpty()) {
        ...
        if ((contentDisposition.count() > 0) && (contentDisposition.first() == QString("attachment"))) {
            ...
            OASkycopeHttpFileElement felement;
...
QByteArray OASkycopeHttpRequestWorker::decompress(const QByteArray& data){
    
    Q_UNUSED(data);
    return QByteArray();
}

QByteArray OASkycopeHttpRequestWorker::compress(const QByteArray& input, int level, OASkycopeCompressionType compressType) {
...
QSslConfiguration *OASkycopeHttpRequestWorker::sslDefaultConfiguration;
