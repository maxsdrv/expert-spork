...
constexpr quint32 compassDisplayStep = 3;
        ...
        if (isCoordinateDrone(device))
        {
            ...
            if (!zeroCoord)
            {
                ...
                emit targetDetected(
                    ...
                    std::nullopt,
                    static_cast<qint64>(device.getSeenTimes()));
            }
    ...
    const QMap<QString, OASkycopeDroneWifiDevice>& consoles)
{
    for (auto it = consoles.keyValueBegin(); it != consoles.keyValueEnd(); ++it)
    {
        ...
        emit targetDetected(
            ...
            correctCoord(qGeoCoordinate(device.getGps())),
            std::nullopt,
            static_cast<qint64>(device.getDetectCounter()));
    }
}
    ...
    core::DevicePosition position;
    {
        ...
        position = core::DevicePosition{
            .azimuth = static_cast<double>(
                sensor.getCompass() - sensor.getCompass() % compassDisplayStep),
