...
bool TamerlanJammer::setBands(const core::JammerBandOption& bandsActive)
{
    for (auto& band: m_bandData)
    {
        ...
        setSuppression(band, it != bandsActive.end());
    }
...
void TamerlanJammer::setSuppression(BandData& bandData, bool value)
{
    auto index = bandData.params.number - 1;

    if (std::cmp_less(index, m_rawData->channelsCount))
    {
        bandData.emissionToSet = value ? 0x01 : 0x00;
        m_executor->writeCoils(
            QUuid::createUuid(),
            channelAddress(static_cast<int>(index))
                + proto::JammerChannelControl::relativeAddress,
            proto::JammerChannelControl::size,
            &(bandData.emissionToSet),
            ModbusServerAddress(m_rawData->modbusAddress));

        core::auditLogger().logSuppressionManualRequested(
            core::JammerBand{bandData.params.name, value}, nodeKey, m_serial);
    }
}
