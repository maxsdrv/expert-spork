...
constexpr auto minDetectionCountTag = "min_detection_count";
...
{
    static std::map<uint, std::shared_ptr<AsyncModbusCommandExecutor>> executors;
    std::map<ConnectionId, uint> executorIds;
    ...
    if (!connectionsNode.IsNull() && connectionsNode.size() > 0)
    {
        ...
        for (const auto& connectionNode: connectionNodes)
        {
            ...
            config::ConnectionSettings tcpSettings;
            tamerlan::ModbusRtuSettings rtuSettings;
            uint hash = 0;
            if (type == "tcp")
            {
                tcpSettings = core::convertYAML<config::ConnectionSettings>(
                    connectionNode);
                hash = tcpSettings.toHash();
            }
            else if (type == "rtu")
            {
                rtuSettings = tamerlan::ModbusRtuSettings{
            ...
            if (executorIds.find(id.value) != executorIds.end())
            {
                BOOST_THROW_EXCEPTION(
                    exception::InvalidConfig() << ::exception::ExceptionInfo(
            ...
            executorIds[id.value] = hash;

            if (executors.find(hash) == executors.end())
            {
                ...
                executors.insert({hash, std::move(executor)});
            }
        }
    }
    ...
    if (!sensorsNode.IsNull() && sensorsNode.size() > 0)
    {
        ...
        for (const auto& sensorNode: sensorNodes)
        {
            ...
            auto minDetectionCount = core::convertYAML<config::Option<int>>(
                sensorNode[minDetectionCountTag]);
            auto connectionId = core::convertYAML<config::Option<ConnectionId>>(
                sensorNode[connectionTag]);
            auto executorHashIt = executorIds.find(connectionId.value);
            if (executorHashIt == executorIds.end())
            {
                BOOST_THROW_EXCEPTION(
                    exception::InvalidConfig() << ::exception::ExceptionInfo(
            ...
            auto executorIt = executors.find(executorHashIt->second);
            QString model;
