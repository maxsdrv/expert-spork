...
BOOST_AUTO_TEST_CASE(volid_lic_file, *boost::unit_test::disabled()) {
...
BOOST_AUTO_TEST_CASE(strategy_mac_address, *boost::unit_test::disabled()) {
...
BOOST_AUTO_TEST_CASE(strategy_ip_address, *boost::unit_test::disabled()) {
	vector<os::OsAdapterInfo> adapters;
	FUNCTION_RETURN result_adapterInfos = os::getAdapterInfos(adapters);
	if ((result_adapterInfos == FUNC_RET_BUFFER_TOO_SMALL || result_adapterInfos == FUNC_RET_OK) &&
		adapters.size() > 0) {
