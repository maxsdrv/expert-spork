...
Ethernet::Interfaces Ethernet::split(const std::string &string, char delimiter) {
	Interfaces result;

	size_t posStart = 0;
	size_t posEnd;

	const size_t delimLength = 1;

	while ((posEnd = string.find(delimiter, posStart)) != std::string::npos) {
		result.insert(string.substr(posStart, posEnd - posStart));
		posStart = posEnd + delimLength;
	}

	result.insert(string.substr(posStart));
	return result;
}

Ethernet::InterfacesOpt Ethernet::getInterfacesFromEnv() {
	static const char *interface_env = getenv(LICENSE_ETHERNET_INTERFACE_ENV_VAR);

	if (interface_env == nullptr) {
		return std::nullopt;
	}

	return split(interface_env);
}
