...
#include "../../hw_identifier/ethernet.hpp"
...
using Ethernet = hw_identifier::Ethernet;

bool isVirtual(const char *interfaceName) {
	static const char virtual_interfaces[][IFNAMSIZ] = {"tun", "tap", "veth", "docker", "br", "virbr"};
	static const size_t count = sizeof(virtual_interfaces) / IFNAMSIZ;

	for (size_t i = 0; i < count; i++) {
		const char *virtual_interface = virtual_interfaces[i];
		const size_t lenght = strlen(virtual_interface);
		if (strncmp(interfaceName, virtual_interface, lenght) == 0) {
			return true;
		}
	}
...
bool isExcluded(const char *interfaceName, Ethernet::InterfacesOpt &interfaces) {
	if (interfaces.has_value()) {
		const bool found = interfaces.value().count(interfaceName) > 0;
		if (found) {
	...
	return isVirtual(interfaceName);
}
...
FUNCTION_RETURN getAdapterInfos(vector<OsAdapterInfo> &adapterInfos) {
	...
	auto interfacesFromEnvOpt = Ethernet::getInterfacesFromEnv();

	for (ifa = ifaddr, n = 0; ifa != NULL; ifa = ifa->ifa_next, n++) {
		const bool if_excluded = isExcluded(ifa->ifa_name, interfacesFromEnvOpt);

		if (ifa->ifa_addr == NULL || (ifa->ifa_flags & IFF_LOOPBACK) != 0 || if_excluded) {
			continue;
		}
		...
		if (family == AF_INET) {
			...
			if (sdl->sdl_alen == 6) {
				...
				for (int i = 0; i < 6; i++) {
					currentAdapter->mac_address[i] = mac[i];
				}
			}
		}
	}
	...
	if (interfacesFromEnvOpt.has_value() && !interfacesFromEnvOpt.value().empty()) {
		for (const std::string &interface : interfacesFromEnvOpt.value()) {
			std::cout << "ERROR: interface " << interface << " not found" << std::endl;
		}
	}
