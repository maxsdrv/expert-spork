# Auto-generated DDS Provider HTTP API Models
...
class CommonGetApiVersionGetResponse(BaseModel):
    api_version: str = Field(..., description='Version string', examples=['1.12.7'])


class SensorGetSensorInfoGetParametersQuery(BaseModel):
    sensor_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )


class SensorGetJammerModeGetParametersQuery(SensorGetSensorInfoGetParametersQuery):
    pass


class SensorGetJammerModeGetResponse(BaseModel):
    jammer_mode: JammerMode


class SensorSetJammerModePostRequest(BaseModel):
    sensor_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    jammer_mode: JammerMode
...
class SensorGetJammerTimeoutGetParametersQuery(SensorGetSensorInfoGetParametersQuery):
    pass


class SensorGetJammerTimeoutGetResponse(BaseModel):
    timeout: int


class SensorSetJammerTimeoutPostRequest(BaseModel):
    sensor_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    timeout: int
...
class SensorSetSensorDisabledPostRequest(BaseModel):
    sensor_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    disabled: bool
...
class JammerGetJammerInfoGetParametersQuery(BaseModel):
    jammer_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )


class JammerSetJammerDisabledPostRequest(BaseModel):
    jammer_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    disabled: bool
...
class JammerSetJammerBandsPostRequest(BaseModel):
    jammer_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
...
class TargetGetTargetDataGetParametersQuery(BaseModel):
    target_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )


class TargetGetTrackGetParametersQuery(BaseModel):
    track_id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )


class CameraGetOrientationGetParametersQuery(BaseModel):
    id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )


class CameraSetGeoDirectionPostRequest(BaseModel):
    id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    geo_direction: GeoDirection


class CameraSetFollowTrackPostRequest(BaseModel):
    id: UUID = Field(
        ..., description='Matching sensor ID identification string based on UUID'
    )
    track_id: UUID = Field(
...
class SensorGetSensorInfoGetResponse(BaseModel):
    sensor_info: SensorInfo


class JammerGetJammerInfoGetResponse(BaseModel):
    jammer_info: JammerInfo
