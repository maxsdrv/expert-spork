...
func (s *Handlers) SetJammerMode(
	...
	err := s.controllers.SetJammerMode(ctx, req.Msg.GetSensorId(), jammerMode, timeout)
	if err != nil {
		logger.WithError(err).Error("Set jammer mode failed")
		return nil, err
	}

	logger.Infof("Successfully set jammer mode for sensor %s to %s", req.Msg.GetSensorId(), jammerMode)
